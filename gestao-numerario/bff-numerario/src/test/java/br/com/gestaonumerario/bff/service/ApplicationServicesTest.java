...
class ApplicationServicesTest {
    ...
    void deveDelegarServicosDeAgencia() {
        ...
        service.listar("Bearer token", "centro", true, false, "codigo", "asc", 0, 20);
        service.detalhar("Bearer token", 1L);
        service.criar("Bearer token", criar);
        service.atualizar("Bearer token", 1L, atualizar);
        service.desativar("Bearer token", 1L);

        verify(client).listarAgencias(
                "Bearer token", "centro", true, false, "codigo", "asc", 0, 20);
        verify(client).detalharAgencia("Bearer token", 1L);
        verify(client).criarAgencia("Bearer token", criar);
        verify(client).atualizarAgencia("Bearer token", 1L, atualizar);
        verify(client).desativarAgencia("Bearer token", 1L);
    }
    ...
    void deveDelegarServicosDeAutenticacao() {
        ...
        service.renovar("refresh");
        service.encerrar("refresh");
        service.consultarSessao("Bearer token");

        verify(client).autenticar(login);
        verify(client).renovar("refresh");
        verify(client).encerrar("refresh");
    ...
    void deveDelegarServicosDeSolicitacao() {
        ...
        service.listar("Bearer token", 1L, "PENDENTE", inicio, fim, 0, 20);
        service.criar("Bearer token", criar);
        service.aprovar("Bearer token", 1L, aprovar);
        service.rejeitar("Bearer token", 1L, rejeitar);
        service.atender("Bearer token", 1L, atender);

        verify(client).listarSolicitacoes(
                "Bearer token", 1L, "PENDENTE", inicio, fim, 0, 20);
        verify(client).criarSolicitacao("Bearer token", criar);
        verify(client).aprovarSolicitacao("Bearer token", 1L, aprovar);
        verify(client).rejeitarSolicitacao("Bearer token", 1L, rejeitar);
        verify(client).atenderSolicitacao("Bearer token", 1L, atender);
    }
    ...
    void deveDelegarMovimentacaoEDashboard() {
        ...
        RegistrarMovimentacaoRequest criar = mock(RegistrarMovimentacaoRequest.class);
        LocalDate data = LocalDate.of(2026, 7, 25);

        movimentacao.listar("Bearer token", 1L, "DEPOSITO", data, data, 0, 20);
        movimentacao.criar("Bearer token", criar);
        dashboard.consultar("Bearer token");

        verify(client).listarMovimentacoes(
                "Bearer token", 1L, "DEPOSITO", data, data, 0, 20);
        verify(client).criarMovimentacao("Bearer token", criar);
