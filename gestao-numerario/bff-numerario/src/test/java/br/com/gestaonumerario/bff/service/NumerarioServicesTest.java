...
class NumerarioServicesTest {
    ...
    void deveDelegarTodasAsConsultas() {
        ...
        verify(client).listarSolicitacoesNumerario(
                "Bearer token",
                1L,
                "SUPRIMENTO",
                "PENDENTE",
        ...
        verify(client).detalharSolicitacaoNumerario(
                "Bearer token",
                4L
        );
        verify(client).consultarHistoricoSolicitacao(
                "Bearer token",
                4L
        );
        verify(client).listarUnidadesOperacionais(
                "Bearer token",
                "AGENCIA"
        );
        verify(client).listarOperacoesNumerario(
                "Bearer token",
                "PROGRAMADA",
                2L,
                3L,
    ...
    void deveDelegarTodosOsComandos() {
        ...
        verify(client).criarSolicitacaoNumerario(
                "Bearer token",
                criar
        );
        verify(client).decidirSolicitacaoNumerario(
                "Bearer token",
                1L,
                "aprovar",
                decidir
        );
        verify(client).programarOperacao(
                "Bearer token",
                1L,
                "p-1",
                programar
        );
        verify(client).iniciarSeparacao(
                "Bearer token",
                1L,
                versao
        );
        verify(client).expedirOperacao(
                "Bearer token",
                1L,
                "e-1",
                executar
        );
        verify(client).registrarOcorrencia(
                "Bearer token",
                1L,
                ocorrencia
        );
        verify(client).receberOperacao(
                "Bearer token",
                1L,
                "r-1",
                receber
        );
        verify(client).conciliarOperacao(
                "Bearer token",
                1L,
                "c-1",
                conciliar
        );
        verify(client).realizarCargaInicial(
                "Bearer token",
                "t-1",
                carga
        );
        verify(client).ajustarDivergencia(
                "Bearer token",
                1L,
                "a-1",
                ajuste
