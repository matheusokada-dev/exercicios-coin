...
public class FinanceiroNumerarioUseCase implements FinanceiroNumerarioInputPort {
    ...
    public FinanceiroNumerarioUseCase(
    ...
    public UnidadeOperacional realizarCargaInicial(CargaInicialTesourariaCommand c) {
        ...
        return transacao.executar(() -> {
            ...
            if (port.existeComandoDoTipo(CARGA_INICIAL)) {
                throw new RegraOperacaoNumerarioException("A carga inicial da Tesouraria já foi realizada.");
            }
            var tesourarias = port.consultarUnidadesAtivas(TipoUnidadeOperacional.TESOURARIA);
            if (tesourarias.size() != 1) {
                throw new RegraOperacaoNumerarioException("A carga inicial exige exatamente uma Tesouraria ativa.");
            }
            var bloqueada = port.buscarUnidadeParaAtualizacao(
                    tesourarias.getFirst()
                            .getId()
            )
            ...
            var salva = port.salvarAjusteFinanceiro(
                    bloqueada,
                    null,
                    ...
                    c.usuarioId(),
                    c.idempotencyKey(),
                    "Carga inicial da Tesouraria: " + c.justificativa()
                            .trim(),
                    agora
            );
    ...
    public UnidadeOperacional ajustarDivergencia(AjustarDivergenciaCommand c) {
        ...
        return transacao.executar(() -> {
            ...
            if (operacao == null || operacao.getValorDivergencia() == null || operacao.getValorDivergencia()
                    .signum() <= 0
            ...
            boolean pertenceRota = operacao.getOrigem()
                    .getId()
                    .equals(c.unidadeId())
                    || operacao.getDestino()
                            .getId()
                            .equals(c.unidadeId());
            if (!pertenceRota || c.valor() == null || c.valor()
                    .compareTo(operacao.getValorDivergencia()) > 0) {
                throw new RegraOperacaoNumerarioException(
                        !pertenceRota
            ...
            var salva = port.salvarAjusteFinanceiro(
                    unidade,
                    operacao.getId(),
                    TipoMovimentacao.AJUSTE_DIVERGENCIA,
                    c.entrada(),
                    c.valor(),
                    c.usuarioId(),
                    c.idempotencyKey(),
