...
public class FinanceiroNumerarioController implements FinanceiroNumerarioApi {
    ...
    public UnidadeOperacionalResponse cargaInicial(
        ...
        return resposta(useCase.realizarCargaInicial(new CargaInicialTesourariaCommand(
                r.valor(),r.justificativa(),r.versaoUnidade(),u.id(),key)));
    }

    @PostMapping("/solicitacoes-numerario/{id}/ajustes-divergencia")
    public UnidadeOperacionalResponse ajustar(@PathVariable Long id,
            @RequestBody AjustarDivergenciaRequest r,
            @RequestHeader(IDEMPOTENCY_KEY) String key,
            @AuthenticationPrincipal UsuarioAutenticado u) {
        return resposta(useCase.ajustarDivergencia(new AjustarDivergenciaCommand(
                id,r.unidadeId(),r.valor(),r.entrada(),r.justificativa(),
                r.versaoUnidade(),u.id(),key)));
    }

    private static UnidadeOperacionalResponse resposta(UnidadeOperacional u) {
        return new UnidadeOperacionalResponse(u.getId(),u.getTipo(),u.getCodigo(),u.getNome(),
                u.getSaldoAtual(),u.getVersao(),u.getAtualizadoEm());
    }
}
