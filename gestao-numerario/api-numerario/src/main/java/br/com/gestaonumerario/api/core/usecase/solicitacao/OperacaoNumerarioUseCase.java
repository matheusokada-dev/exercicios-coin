...
public class OperacaoNumerarioUseCase implements OperacaoNumerarioInputPort {
    ...
    public OperacaoNumerarioUseCase(NumerarioOutputPort port,UsuarioOutputPort usuarios,
    ...
    public OperacaoNumerario programar(ProgramarOperacaoNumerarioCommand c) {
        validar(c,c==null?null:c.solicitacaoId(),c==null?null:c.unidadeFaltanteId(),
        ...
        return transacao.executar(() -> {
            ...
            var operacao=detalhe.solicitacao().programar(unidade,usuario(c.usuarioId()),
                    c.idempotencyKey(),c.versaoSolicitacao(),relogio.agora());
            var salva=port.salvarProgramacao(operacao);
            port.registrarIdempotencia(c.idempotencyKey(),"PROGRAMAR",salva.getId(),
                    c.usuarioId(),relogio.agora());
            return salva;
        });
    }
    ...
    public OperacaoNumerario iniciarSeparacao(ExecutarOperacaoNumerarioCommand c) {
        validar(c,c==null?null:c.solicitacaoId(),c==null?null:c.usuarioId());
        return transacao.executar(() -> {
            var o=operacao(c.solicitacaoId());
            o.iniciarSeparacao(usuario(c.usuarioId()),c.versaoOperacao(),relogio.agora());
    ...
    public OperacaoNumerario expedir(ExecutarOperacaoNumerarioCommand c) {
        ...
        return transacao.executar(() -> {
            rejeitarChaveRepetida(c.idempotencyKey());
            var o=operacao(c.solicitacaoId());
            port.buscarUnidadeParaAtualizacao(o.getOrigem().getId())
                    .orElseThrow(RegraOperacaoNumerarioException::new);
            var agora=relogio.agora();
            o.expedir(usuario(c.usuarioId()),c.versaoOperacao(),c.versaoUnidade(),
                    c.idempotencyKey(),agora);
            var salva=port.salvarOperacaoFinanceira(o,TipoMovimentacao.SAIDA_PARA_TRANSITO,
                    c.usuarioId(),c.idempotencyKey(),"Expedição da solicitação "+c.solicitacaoId(),agora);
            port.registrarIdempotencia(c.idempotencyKey(),"EXPEDIR",salva.getId(),
                    c.usuarioId(),agora);
            return salva;
        });
    }
    ...
    public OperacaoNumerario registrarOcorrencia(RegistrarOcorrenciaOperacaoCommand c) {
        ...
        return transacao.executar(() -> {
            var o=operacao(c.solicitacaoId());
            o.registrarOcorrencia(c.descricao(),usuario(c.usuarioId()),
                    c.versaoOperacao(),relogio.agora());
            return port.salvarOperacao(o);
        });
    }
    ...
    public OperacaoNumerario receber(ReceberOperacaoNumerarioCommand c) {
        ...
        return transacao.executar(() -> {
            rejeitarChaveRepetida(c.idempotencyKey());
            var o=operacao(c.solicitacaoId());
            port.buscarUnidadeParaAtualizacao(o.getDestino().getId())
                    .orElseThrow(RegraOperacaoNumerarioException::new);
            var agora=relogio.agora();
            o.receber(c.valorRecebido(),c.justificativaDivergencia(),usuario(c.usuarioId()),
                    c.versaoOperacao(),c.versaoUnidade(),c.idempotencyKey(),agora);
            var salva=port.salvarOperacaoFinanceira(o,TipoMovimentacao.ENTRADA_DE_TRANSITO,
                    c.usuarioId(),c.idempotencyKey(),"Recebimento da solicitação "+c.solicitacaoId(),agora);
            port.registrarIdempotencia(c.idempotencyKey(),"RECEBER",salva.getId(),
                    c.usuarioId(),agora);
            return salva;
        });
    }
    ...
    public OperacaoNumerario conciliar(ConciliarOperacaoNumerarioCommand c) {
        ...
        return transacao.executar(() -> {
            ...
            var o=operacao(c.solicitacaoId());
            var agora=relogio.agora();
            o.conciliar(c.justificativa(),usuario(c.usuarioId()),c.versaoOperacao(),
                    c.idempotencyKey(),agora);
            var salva=port.salvarOperacao(o);
            port.registrarIdempotencia(c.idempotencyKey(),"CONCILIAR",salva.getId(),
                    c.usuarioId(),agora);
            return salva;
        });
    }
    ...
    private DetalheSolicitacaoNumerario detalhe(Long id) {
        return port.buscarDetalhe(id).orElseThrow(SolicitacaoNaoEncontradaException::new);
    }
    ...
    private Usuario usuario(Long id) {
        var usuario=usuarios.buscarPorId(id).orElseThrow(UsuarioNaoEncontradoException::new);
        if (!usuario.isAtivo()) {
            throw new RegraOperacaoNumerarioException(
    ...
    private static void validar(Object c,Long... ids) {
        ...
        for (Long id : ids) {
            if (id == null) throw new CampoObrigatorioException("identificador");
        }
    }
}
