...
public class AgenciaUseCase implements AgenciaInputPort {
    ...
    public AgenciaUseCase(AgenciaOutputPort agenciaPort,
    ...
    public Agencia criar(CriarAgenciaCommand command) {
        ...
        return transacaoPort.executar(() -> {
            ...
            return agenciaPort.salvar(new Agencia(null, codigo, command.nome(), command.cidade(),
                    command.saldoAtual(), command.limiteMinimo(), true, 0));
        });
    }
    ...
    public Agencia atualizar(AtualizarAgenciaCommand command) {
        ...
        return transacaoPort.executar(() -> {
            ...
            agencia.atualizarDados(command.nome(), command.cidade(), command.limiteMinimo());
    ...
    public DetalheAgencia detalhar(Long agenciaId) {
        ...
        ResumoMovimentacaoDiaria resumo = movimentacaoPort.resumirDiaPorAgencia(agenciaId, hoje);
        ...
        return new DetalheAgencia(agencia, hoje, resumo.valorEntradas(), resumo.valorSaidas(),
                valorAprovado, ValorMonetario.normalizar(agencia.getSaldoAtual().add(valorAprovado)));
    }
    ...
    private Agencia buscarAgencia(Long id) {
        return agenciaPort.buscarPorId(id).orElseThrow(AgenciaNaoEncontradaException::new);
    }
