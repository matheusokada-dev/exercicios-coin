...
public class OperacaoNumerarioController implements OperacaoNumerarioApi {
    ...
    public OperacaoNumerarioResponse programar(@PathVariable Long id,
        ...
        return mapper.toResponse(useCase.programar(new ProgramarOperacaoNumerarioCommand(
                id,r.unidadeFaltanteId(),r.versaoSolicitacao(),u.id(),key)));
    }
    @PutMapping("/{id}/iniciar-separacao")
    public OperacaoNumerarioResponse separar(@PathVariable Long id,
            @RequestBody VersaoOperacaoNumerarioRequest r,
            @AuthenticationPrincipal UsuarioAutenticado u) {
        return mapper.toResponse(useCase.iniciarSeparacao(new ExecutarOperacaoNumerarioCommand(
                id,r.versaoOperacao(),0,u.id(),null)));
    }
    @PutMapping("/{id}/expedir")
    public OperacaoNumerarioResponse expedir(@PathVariable Long id,
            @RequestBody ExecutarOperacaoNumerarioRequest r,
            @RequestHeader(IDEMPOTENCY_KEY) String key,
            @AuthenticationPrincipal UsuarioAutenticado u) {
        return mapper.toResponse(useCase.expedir(new ExecutarOperacaoNumerarioCommand(
                id,r.versaoOperacao(),r.versaoUnidade(),u.id(),key)));
    }
    @PutMapping("/{id}/registrar-ocorrencia")
    public OperacaoNumerarioResponse ocorrencia(@PathVariable Long id,
            @RequestBody OcorrenciaOperacaoNumerarioRequest r,
            @AuthenticationPrincipal UsuarioAutenticado u) {
        return mapper.toResponse(useCase.registrarOcorrencia(
                new RegistrarOcorrenciaOperacaoCommand(id,r.descricao(),r.versaoOperacao(),u.id())));
    }
    @PutMapping("/{id}/receber")
    public OperacaoNumerarioResponse receber(@PathVariable Long id,
            @RequestBody ReceberOperacaoNumerarioRequest r,
            @RequestHeader(IDEMPOTENCY_KEY) String key,
            @AuthenticationPrincipal UsuarioAutenticado u) {
        return mapper.toResponse(useCase.receber(new ReceberOperacaoNumerarioCommand(
                id,r.valorRecebido(),r.justificativaDivergencia(),r.versaoOperacao(),
                r.versaoUnidade(),u.id(),key)));
    }
    @PutMapping("/{id}/conciliar")
    public OperacaoNumerarioResponse conciliar(@PathVariable Long id,
            @RequestBody ConciliarOperacaoNumerarioRequest r,
            @RequestHeader(IDEMPOTENCY_KEY) String key,
            @AuthenticationPrincipal UsuarioAutenticado u) {
        return mapper.toResponse(useCase.conciliar(new ConciliarOperacaoNumerarioCommand(
                id,r.justificativa(),r.versaoOperacao(),u.id(),key)));
    }
}
