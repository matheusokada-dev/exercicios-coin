...
public class JwtTokenService implements TokenJwtOutputPort {
    ...
    public TokenAcesso gerar(Usuario usuario) {
        ...
        String token = Jwts.builder().subject(usuario.getLogin()).claim("uid", usuario.getId())
                .claim("perfil", usuario.getPerfil().name()).issuedAt(Date.from(agora))
                .expiration(Date.from(expiraEm)).signWith(key).compact();
        return new TokenAcesso(token, expiraEm);
    }

    public UsuarioAutenticado validar(String token) {
        var claims = Jwts.parser().verifyWith(key).build().parseSignedClaims(token).getPayload();
        return new UsuarioAutenticado(claims.get("uid", Long.class), claims.getSubject(),
                PerfilUsuario.valueOf(claims.get("perfil", String.class)));
    }
}
