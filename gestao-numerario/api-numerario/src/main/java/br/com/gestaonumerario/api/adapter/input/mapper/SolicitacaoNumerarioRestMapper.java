...
public class SolicitacaoNumerarioRestMapper {
    public OperacaoNumerarioResponse toResponse(OperacaoNumerario o) {
        return new OperacaoNumerarioResponse(
                o.getId(),
                o.getSolicitacao()
                        .getId(),
                o.getOrigem()
    ...
    public SolicitacaoNumerarioResponse toResponse(SolicitacaoNumerario s) {
        return new SolicitacaoNumerarioResponse(
                s.getId(),
                s.getTipoOperacao(),
                s.getAgenciaReferenciaId(),
                s.getOrigem() == null
                        ? null
                        : s.getOrigem()
                                .getId(),
    ...
    public DetalheSolicitacaoNumerarioResponse toResponse(DetalheSolicitacaoNumerario d) {
        ...
        var operacao = o == null
                ? null
                : new DetalheSolicitacaoNumerarioResponse.OperacaoResumo(
                        o.getId(),
                        o.getStatus(),
                        o.getOrigem()
                                .getId(),
        ...
        var historico = d.historico()
                .stream()
                .map(
                        h -> new DetalheSolicitacaoNumerarioResponse.HistoricoResumo(
                                h.id(),
                                h.evento(),
                                h.statusAnterior(),
                                h.statusNovo(),
                                ...
                                h.dataEvento(),
                                h.justificativa(),
                                h.dadosComplementares()
                        )
                )
                .toList();
        return new DetalheSolicitacaoNumerarioResponse(
                toResponse(d.solicitacao()),
                operacao,
                historico
        );
    }
