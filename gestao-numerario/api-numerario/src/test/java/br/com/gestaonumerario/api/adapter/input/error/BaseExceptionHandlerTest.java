...
class BaseExceptionHandlerTest {
    ...
    void retornaContratoRestCompletoSemRemoverCamposLegados() {
        ...
        var response = handler.handleBaseException(
                new AgenciaNaoEncontradaException(),
                request
        );
        ...
        assertThat(
                response.getBody()
                        .getCode()
        ).isEqualTo("AGENCIA_NAO_ENCONTRADA");
        assertThat(
                response.getBody()
                        .getPath()
        ).isEqualTo("/api/v1/agencias/999");
        assertThat(
                response.getBody()
                        .getTimestamp()
        ).isNotNull();
    ...
    void identificaCampoComTipoInvalido() {
        ...
        var exception = new MethodArgumentTypeMismatchException(
                ...
                "agenciaId",
                null,
                new IllegalArgumentException("tipo inválido")
        );
    ...
    void preservaCampoERegraEspecificosDoDominio() {
        ...
        assertThat(
                campo.getBody()
                        .getMessage()
        ).isEqualTo("O campo 'agenciaId' é obrigatório.");
        assertThat(
                campo.getBody()
                        .getFields()
        ).containsExactly(
        ...
        var regra = handler.handleBaseException(
                new RegraOperacaoNumerarioException("A carga inicial da Tesouraria já foi realizada."),
                new MockHttpServletRequest(
                        "POST",
        ...
        assertThat(
                regra.getBody()
                        .getMessage()
        ).isEqualTo("A carga inicial da Tesouraria já foi realizada.");
    }
    ...
    void informaTentativasRestantesEmCredenciaisInvalidas() {
        ...
        assertThat(
                response.getBody()
                        .getValue()
                        .bloqueadoAte()
        ).isEqualTo(bloqueadoAte);
        assertThat(
                handler.handleCredenciaisInvalidas(
                        new CredenciaisInvalidasException(),
                        new MockHttpServletRequest()
                )
                        .getBody()
    ...
    void converteBeanValidationEmContratoPadronizado() throws NoSuchMethodException {
        var binding = new BeanPropertyBindingResult(
                new Object(),
                "request"
        );
        binding.addError(
    ...
    private static void adicionarErro(BeanPropertyBindingResult binding, String campo, String codigo) {
        binding.addError(
                new FieldError(
                        "request",
                        campo,
